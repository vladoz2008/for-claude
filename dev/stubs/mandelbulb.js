// DEV STUB — stands in for src/scenes/mandelbulb.js while the shell is being
// built and screenshotted. Not part of the shipped product. Same mount/unmount
// contract as a real scene, minus the real ray-marched fractal.
(function () {
  'use strict';
...
void main() {
  ...
  SCENES.mandelbulb = {
    id: 'mandelbulb',
    name: 'Мандельбульб',
    tagline: 'Заглушка для проверки вёрстки оболочки — не боевая сцена 3D-фрактала.',
    about:
      'Черновая заглушка: лепестковый sin-узор на полноэкранном WebGL2-шейдере имитирует ' +
      'изрезанную поверхность фрактала. Нужна только затем, чтобы увидеть, как реальный канвас ' +
      'выглядит под интерфейсом оболочки, прежде чем настоящая сцена будет готова.',
    spec: [
      ['Рендер', 'WebGL2, 1 draw call'],
      ['Разрешение', 'адаптивное 0.3–1.0'],
      ['Узор', 'лепестки × кольца'],
      ['Имитация', 'дистанционное поле'],
      ['DPR', 'min(devicePixelRatio, 2)']
    ],
    hint: 'Тяните — вращение · колесо или щипок — зум',
    accent: '#b98cf2',
    mount(host, ui) {
      return createStub(host, ui, FRAG, hexToRgb01('#b98cf2'));
    }
  };
