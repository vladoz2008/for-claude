// DEV STUB — stands in for src/scenes/gargantua.js while the shell is being
// built and screenshotted. Not part of the shipped product. Same mount/unmount
// contract as a real scene, minus the real physics.
(function () {
  'use strict';
...
void main() {
  ...
  SCENES.gargantua = {
    id: 'gargantua',
    name: 'Гаргантюа',
    tagline: 'Заглушка для проверки вёрстки оболочки — не боевая сцена чёрной дыры.',
    about:
      'Черновая заглушка: вихревой градиент на полноэкранном WebGL2-шейдере имитирует свечение ' +
      'аккреционного диска. Нужна только затем, чтобы увидеть, как реальный канвас выглядит под ' +
      'интерфейсом оболочки, прежде чем настоящая сцена будет готова.',
    spec: [
      ['Рендер', 'WebGL2, 1 draw call'],
      ['Разрешение', 'адаптивное 0.3–1.0'],
      ['Узор ядра', 'sin-вихрь'],
      ['Диск', 'имитация свечения'],
      ['DPR', 'min(devicePixelRatio, 2)']
    ],
    hint: 'Тяните — сдвиг обзора · колесо или щипок — зум',
    accent: '#f2b880',
    mount(host, ui) {
      return createStub(host, ui, FRAG, hexToRgb01('#f2b880'));
    }
  };
